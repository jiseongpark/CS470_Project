    ...
    for epoch in range(args.start_epoch, 180):
        ...
        if acc > best_acc:
            ...
            if not os.path.isdir('checkpoint'):
                os.mkdir('checkpoint')
#             torch.save(state, './checkpoint/ckpt_cifar_pi2.t7')
            best_acc = acc
...
#         transforms.Normalize((0.4377, 0.4438, 0.4728), (0.1201, 0.1231, 0.1052)),
    ...
    label_loader = torch.utils.data.DataLoader(labeled, batch_size=256, shuffle=True, num_workers=4, drop_last=False)
...
def train(label_loader, unlabel_loader, model,ema_model, optimizer, epoch):
    ...
    for i in range(len_iter):
        ...
        try:
            input_l, target, input1_l = next(label_iter)
        ...
        input_var = torch.autograd.Variable(input_l)
        input1_var = torch.autograd.Variable(input1_l)
        input_ul_var = torch.autograd.Variable(input_ul)
        input1_ul_var = torch.autograd.Variable(input1_ul)
        input_concat_var = torch.cat([input_var, input_ul_var]).to('cuda')
        input1_concat_var = torch.cat([input1_var, input1_ul_var]).to('cuda')
        target_var = torch.autograd.Variable(target).to('cuda')
        
        model_out = model(input_concat_var)
        ema_model_out = ema_model(input1_concat_var)
        with torch.no_grad():
            output1 = model(input1_concat_var)
        output_label = model_out[:sl[0]]
        class_loss = class_criterion(output_label, target_var) / float(sl[0])
        consistency_loss = consistency_weight * consistency_criterion(model_out, ema_model_out) / batch_size
